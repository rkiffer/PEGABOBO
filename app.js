...
function mapInfo() {
  return MAPS.find(m => m.id === currentMap) || MAPS[0];
}
...
function renderSpots() {
  els.spotsLayer.innerHTML = spots.map(s => {
    ...
    return `
      <div 
        class="spot${hidden}${selectedSpotId === s.id ? " selected" : ""}" 
        data-id="${s.id}" 
...
/*
  IMPORTANT:
  This function calculates the click position based on the real displayed image,
  not the full black map stage. This fixes the offset issue.
*/
function imagePointToPercent(event) {
...
function isClickInsideImage(event) {
  const rect = els.mapImage.getBoundingClientRect();
...
function syncSpotsLayerToImage() {
  const imageRect = els.mapImage.getBoundingClientRect();
  const stageRect = els.mapStage.getBoundingClientRect();

  els.spotsLayer.style.left = `${imageRect.left - stageRect.left}px`;
  els.spotsLayer.style.top = `${imageRect.top - stageRect.top}px`;
  els.spotsLayer.style.width = `${imageRect.width}px`;
  els.spotsLayer.style.height = `${imageRect.height}px`;
}

function openSpotDialogForNew(event) {
  if (event.target.closest(".spot")) return;

  if (!isClickInsideImage(event)) return;
  ...
  syncSpotsLayerToImage();

  pendingSpot = {
    ...imagePointToPercent(event),
    mode: "new"
  };
...
function openCharacterDialog(char = null) {
  ...
  els.charModalTitle.textContent = char
    ? "Editar Personagem"
...
els.saveSpotBtn.addEventListener("click", (e) => {
  e.preventDefault();
...
els.addCharacterBtn.addEventListener("click", () => {
  openCharacterDialog();
});

els.saveCharacterBtn.addEventListener("click", (e) => {
  e.preventDefault();
